    ...
    if chat_db:
        ...
        if chat_member.restricted_by:
            if chat_member.restricted_by.id == (await client.get_me()).id:
                try:
                    ...
                    await client.answer_callback_query(cb.id,
                                                       text="❗ Join the mentioned 'channel' and press the 'UnMute Me' button again.",
                                                       show_alert=True)
            else:
                await client.answer_callback_query(cb.id, text="❗ You are muted by admins for other reasons.",
                                                   show_alert=True)
        else:
            if not (await client.get_chat_member(chat_id, (await client.get_me()).id)).status == 'administrator':
                ...
                await client.answer_callback_query(cb.id,
                                                   text="❗ Warning: Don't click the button if you can speak freely.",
                                                   show_alert=True)
...
async def _check_member(client, message):
    ...
    if chat_db:
        ...
        if not (await client.get_chat_member(chat_id, user_id)).status in (
        "administrator", "creator") and not user_id in Config.SUDO_USERS:
            channel = chat_db.channel
            ...
            try:
                ...
                try:
                    sent_message = await message.reply_text(
                        " {} , you are not subscribed to my channel yet. Please join using below button and press the UnMute Me button to unmute yourself.".format(
                ...
                await client.send_message(chat_id,
                                          text=f"❗ **I am not an admin in [channel]({channel_url})**\n__Make me admin in the channel and add me again.\n#Leaving this chat...__")
                await client.leave_chat(chat_id)
            ...
            if input_str.lower() in ("off", "no", "disable"):
                ...
                try:
                    ...
                    await message.reply_text(
                        f"✅ **Force Subscribe is Enabled**\n__Force Subscribe is enabled, all the group members have to subscribe this [channel]({channel_url}) in order to send messages in this group.__",
                        disable_web_page_preview=True)
                except UserNotParticipant:
                    await message.reply_text(
                        f"❗ **Not an Admin in the Channel**\n__I am not an admin in the [channel]({channel_url}). Add me as a admin in order to enable ForceSubscribe.__",
                        disable_web_page_preview=True)
